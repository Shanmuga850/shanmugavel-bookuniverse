...
function HomeContent() {
  ...
  const filteredEbooks = useMemo(() => {
    ...
    return ebooks.filter((book) => {
      const haystack =
        `${book.title} ${book.subtitle || ""} ${book.authors.join(" ")} ${book.sku || ""}`.toLowerCase();
  ...
  const filteredAudiobooks = useMemo(() => {
    ...
    return audiobooks.filter((book) => {
      const haystack =
        `${book.title} ${book.authors.join(" ")} ${book.sku || ""}`.toLowerCase();
      return haystack.includes(query);
    });
  }, [audiobooks, query]);
  ...
  return (
    <div className="min-h-screen vels-bg">
          ...
          <div className="max-w-7xl mx-auto relative z-10">
            <div className="flex flex-col items-center text-center gap-8 animate-fade-in-up">
              ...
              <h1 className="sr-only">
                Shanmugavel's Bookstore - Premium eBooks and Audiobooks for 5%
                Thinkers by Shanmugavel M
              </h1>

              <div className="space-y-4">
                <h2 className="font-serif text-5xl md:text-7xl font-bold gold-text">
                  {founderTagline}
                </h2>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between black-gold-card px-6 py-4">
              <p className="text-sm text-foreground">
                Showing{" "}
                <span className="text-[hsl(43_65%_52%)] font-bold">
                  {totalResults}
                </span>{" "}
                results for{" "}
      ...
      <section
        id="ebooks"
        className={`py-16 px-4 sm:px-6 lg:px-8 ${query ? "pt-6" : ""}`}
      >
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            <BookOpen className="h-6 w-6 text-[hsl(43_65%_52%)]" />
            <h2 className="font-serif text-3xl font-bold gold-text">
              eBooks {query && `(${filteredEbooks.length})`}
            </h2>
          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="black-gold-card aspect-[3/4] animate-pulse"
                />
              ))}
            </div>
            ...
            <div className="black-gold-card p-12 text-center">
              ...
              <p className="text-muted-foreground">
                {query
                  ? `No eBooks found for "${searchParams.get("q")}"`
      ...
      <section id="audiobooks" className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="font-serif text-3xl font-bold gold-text">
              Audiobooks {query && `(${filteredAudiobooks.length})`}
            </h2>
            <div className="flex-1 h-px bg-gradient-to-r from-[hsl(43_30%_25%)] to-transparent" />
          </div>
          ...
          {loading ? (
            ...
            <div className="black-gold-card p-12 text-center">
              ...
              <p className="text-muted-foreground">
                {query
                  ? `No audiobooks found for "${searchParams.get("q")}"`
                  : "No audiobooks published yet. Check back soon."}
              </p>
            </div>
          )}
        </div>
    ...
    <Suspense
      fallback={
        <div className="min-h-screen vels-bg flex items-center justify-center">
          <p className="gold-text">Loading...</p>
        </div>
      }
    >
      <HomeContent />
